...
const StoreDashboard = () => {
  ...
  const generatePDFReport = () => {
    // Safety check: ensure all sub-properties exist before generating
    if (!storeData || !storeData.categoricalData) return;
    ...
    doc.text(`Terminal Sync: ${lastTerminalSync ? new Date(lastTerminalSync).toLocaleString() : 'N/A'}`, 14, 35);

    autoTable(doc, {
      startY: 45,
      head: [['Category', 'On-Hand Stock', '7D Forecast', 'QA Status', 'Operational Status']],
    ...
    doc.setFontSize(9);
    doc.setFont("helvetica", "normal");
    doc.text(`- Local Reliability Score: ${kpis?.forecastMarginMAE || 'N/A'}`, 14, yPos + 8);
    doc.text(`- Quality Assurance: ${kpis?.anomalyCount || 0} Flagged Items in current batch.`, 14, yPos + 13);
    ...
    categoricalData.forEach((c) => {
      if (c.status === 'Critical' || c.status === 'Low') {
        const req = Math.max(50, Math.round((c.demand7d || 0) * 1.5));
  ...
  // Check for loading or null data before rendering main UI
  if (loading || !storeData) {
    return (
      <div className="p-20 text-center animate-pulse text-[#3b82f6] font-black italic uppercase">
        Synchronizing Local Assets...
      </div>
    );
  }

  // Safely extract properties now that we know storeData exists
  const categoricalData = storeData.categoricalData || [];
  const kpis = storeData.kpis || { anomalyCount: 0, forecastMarginMAE: '0%' };
  const transferSuggestions = storeData.transferSuggestions || [];
    ...
    <main className="p-4 md:p-8 space-y-10">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-card-bg/20 p-6 rounded-2xl border border-white/5 shadow-2xl">
        ...
        <button
          ...
          className={`flex items-center gap-2 px-6 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${isGenerating ? 'bg-white/5 text-text-muted' : 'bg-[#3b82f6] text-white hover:scale-105 active:scale-95 shadow-lg shadow-[#3b82f6]/20'
            }`}
        >
                  ...
                  {categoricalData.map((row, i) => (
                    <tr key={i} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                      ...
                      <td className="py-4 px-2 text-right">
                        <span className={`text-[9px] font-black px-2 py-1 rounded uppercase tracking-tighter ${row.status === 'Critical' ? 'bg-red-500/10 text-red-500' :
